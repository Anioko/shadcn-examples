...
interface InnovationDashboardPageProps {
  params: Promise<{
    slug: string
  }>
}
...
export async function generateMetadata({
  params,
}: InnovationDashboardPageProps): Promise<Metadata> {
  const { slug } = await params
  const config = getFrameworkDashboardConfig(slug)

  if (!config) {
    return {
      title: "Innovation Dashboard | ReqArchitect",
    }
  }
...
export default async function InnovationDashboardPage({
  params,
}: InnovationDashboardPageProps) {
  const { slug } = await params
  const config = getFrameworkDashboardConfig(slug)
